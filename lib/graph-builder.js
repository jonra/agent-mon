...
function buildGraph(projects, activeSessionIds, options = {}) {
  ...
  for (const project of projects) {
    ...
    for (const session of sessionsToShow) {
      ...
      // Tool nodes (scoped per-project to avoid global spider-web)
      if (options.showTools !== false) {
        for (const tool of parsed.toolsUsed) {
          const toolNodeId = `tool:${project.dirName}:${tool}`;
  ...
  // BFS: walk edges directionally from active sessions.
  // - From sessions: walk both directions (find parent project + child tools/subagents)
  // - From tool/subagent nodes: stop — don't walk back to other sessions
  let frontier = [...connectedIds];
  while (frontier.length > 0) {
    const next = [];
    for (const id of frontier) {
      const node = nodeMap.get(id);
      // Don't expand from tool or subagent nodes to prevent cross-session leaking
      if (node && (node.type === 'tool' || node.type === 'subagent')) continue;
